  ...
  for (int i = 0; i < lab.rooms.size(); i++) {
    ...
    for (int j = i + 1; j < lab.rooms.size(); j++) {
      ...
      if (lab.hasConnection(fromId, toId)) {
        ...
        modelData.isPlanned = plannedConnections.contains(std::pair<QString, QString>(fromId, toId)) ||
                              plannedConnections.contains(std::pair<QString, QString>(toId, fromId));
        d.append(modelData);
      }
    }
  }
...
void ConnectionModel::updateFromData(const NavigationData& data)
{
  ...
  for (auto i = d.begin(); i != d.end(); i++) {
    i->isPlanned = plannedConnections.contains(std::pair<QString, QString>(i->fromRoom, i->toRoom)) ||
                   plannedConnections.contains(std::pair<QString, QString>(i->toRoom, i->fromRoom));
  }
